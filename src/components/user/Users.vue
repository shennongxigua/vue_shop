<template>
  <div>
      ...
      <el-table :data="userlist" :border="true" :stripe="true">
        ...
        <el-table-column label="状态">
          <template v-slot="slotProps">
            <el-switch v-model="slotProps.row.mg_state" @change="userStatechange(slotProps.row)" >
            </el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="180px">
          <template v-slot="slotProps">
            <!-- 修改按钮 -->
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDailog(slotProps.row.id)"></el-button>
            <!-- 删除按钮 -->
    ...
    <!-- 添加用户的对话框 -->
    <el-dialog title="添加新用户" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      <!-- 内容主体区 -->
    ...
    <!-- 修改用户的对话框 -->
    <el-dialog title="提示" :visible.sync="editDialogVisible"  width="50%" @close="editDialogColose">
      <!-- 内容主体区 -->
    ...
    <!-- 分配角色的对话框 -->
    <el-dialog title="分配角色" :visible.sync="setRoleDialogVisible" width="50%" @close="setRoleDialogColose">
      <div>
...
export default {
  ...
  methods: {
    ...
    async svaeRoleInfo() {
      ...
      if (res.meta.status !== 200) return this.$message.error('服务器出错 修改失败!')
